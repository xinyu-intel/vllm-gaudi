...
class HPUModelRunner(KVConnectorModelRunnerMixin):
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        if len(mamba_layers) > 0:
            ...
            for layer_name, mamba_module in mamba_layers.items():
                kv_cache_spec[layer_name] = MambaSpec(shapes=mamba_module.get_state_shape(),
    ...
    def initialize_attn_backend(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        def get_attn_backends_for_layers(layer_names: list[str]) -> dict[type[AttentionBackend], list[str]]:
            attn_backends = {}
            ...
            for attn_backend, layer_names in attn_backends_map.items():
                ...
                try:
                    ...
                    if "<' not supported between instances of 'NoneType'" in msg or "max_capture_size" in msg:
                        logger.warning("Builder init failed due to missing compilation_config.max_capture_size; "
                                       "injecting conservative fallback and retrying.")
                        orig_comp = getattr(self.vllm_config, 'compilation_config', None)
                        from types import SimpleNamespace
                        fake_comp = SimpleNamespace(
                            max_capture_size=1,
                            static_forward_context=(getattr(orig_comp, 'static_forward_context', {})
                                                    if orig_comp is not None else {}))
